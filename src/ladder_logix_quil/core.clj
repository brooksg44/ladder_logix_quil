...
;; Data structure definitions
 ...
 (defrecord Contact [x y normally-closed? name state branch-id])
 (defrecord Coil [x y name state])
 (defrecord Connection [x1 y1 x2 y2 branch-id])
 (defrecord Node [x y connected-branches]) ; Junction node for parallel branches

;; Sample ladder diagram with parallel branches
 (def initial-state
   {:contacts [(->Contact 150 100 false "X1" false 0)
               (->Contact 300 100 true "X2" false 0)
              ;; Parallel branch 1
               (->Contact 200 160 false "X3" false 1)
              ;; Parallel branch 2
               (->Contact 200 220 true "X4" false 2)
              ;; Parallel branch 3
               (->Contact 200 280 false "X5" false 3)
               (->Contact 350 280 true "X6" false 3)
              ;; Second rung
               (->Contact 150 350 false "X7" false 4)
               (->Contact 300 350 false "Y1" false 4)] ; Feedback from first coil

    :coils [(->Coil 500 190 "Y1" false)
            (->Coil 500 350 "Y2" false)]

    :connections [;; Main horizontal lines
                  (->Connection 50 100 150 100 0)
                  (->Connection 200 100 300 100 0)
                  (->Connection 350 100 400 100 0)

                 ;; Parallel branch 1
                  (->Connection 100 100 100 160 0)
                  (->Connection 100 160 200 160 1)
                  (->Connection 250 160 400 160 1)

                 ;; Parallel branch 2
                  (->Connection 100 160 100 220 0)
                  (->Connection 100 220 200 220 2)
                  (->Connection 250 220 400 220 2)

                 ;; Parallel branch 3
                  (->Connection 100 220 100 280 0)
                  (->Connection 100 280 200 280 3)
                  (->Connection 250 280 350 280 3)
                  (->Connection 400 280 400 220 3)

                 ;; Vertical connection from parallel branches to coil
                  (->Connection 400 100 400 280 0)
                  (->Connection 400 190 500 190 0)
                  (->Connection 550 190 600 190 0)

                 ;; Second rung
                  (->Connection 50 350 150 350 4)
                  (->Connection 200 350 300 350 4)
                  (->Connection 350 350 500 350 4)
                  (->Connection 550 350 600 350 4)

                 ;; Power rails
                  (->Connection 50 100 50 350 0)
                  (->Connection 600 190 600 350 0)]

    :nodes [(->Node 100 160 [0 1])
            (->Node 100 220 [0 2])
            (->Node 100 280 [0 3])
            (->Node 400 100 [0])
            (->Node 400 160 [1])
